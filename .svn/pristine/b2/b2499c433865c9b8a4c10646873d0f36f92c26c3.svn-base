 ...
 **/
template<int DIM>
class DiscreteWorld : public pmTensor<int> {
private:
	pmMatrix<pmAcceleration<float, DIM>*>* _wall_proximity;
public:
	DiscreteWorld(void);
...
public:
	...
	void exportVMF(const std::string&, int = 0);
public:
	void calculateWallProximity(int);
	pmAcceleration<float, DIM>* getProximity(const pmPosition<float, DIM>&);
	inline pmMatrix<pmAcceleration<float, DIM>*>* getWallProximities(void) const { return _wall_proximity; }
private:
...
protected:
	...
	bool _bombsActive;
	std::vector<pmPosition<float, DIM>*> _safezones;
public:
	...
	inline void addSafezone(float x, float y) {
		pmPosition<float, DIM>* zone = new pmPosition<float, DIM>;
		zone->set(x, 0);
		zone->set(y, 1);
		_safezones.push_back(zone);
	}
	inline std::vector<pmPosition<float, DIM>*>* getSafezones() {
		return &_safezones;
	}
...
void DiscreteWorld<DIM>::load(const std::string& filename) {
	std::cout << "Loading world data from: " << filename << "... ";
	...
	if(in.is_open()) {
		...
		if(wallres > 0) {
			...
			_wall_proximity = new pmMatrix<pmAcceleration<float, DIM>*>(ww, wh);
			pmAcceleration<float, DIM>* acc = NULL;
			float tmp;
			for(int y = 0; y < wh; ++y)
				for(int x = 0; x < ww; ++x) {
					acc = new pmAcceleration<float, DIM>;
					for(int i = 0; i < DIM; ++i) {
						in >> tmp;
						acc->set(tmp, i);
					}
...
void DiscreteWorld<DIM>::save(const std::string& filename) {
	std::cout << "Saving world data to: " << filename << "... ";
	...
	if(out.is_open()) {
		...
		for(it = _bombs.begin(); it != _bombs.end(); ++it) {
			pmPosition<float, DIM> bPos = (*it).getpmPosition();
 ...
 **/
template<int DIM>
void DiscreteWorld<DIM>::exportVMF(const std::string& filename, int num_boids) {
	ValveMap<DIM> vmf(this, num_boids);
	vmf.save(filename);
}
 ...
 **/
template<int DIM>
pmAcceleration<float, DIM>* DiscreteWorld<DIM>::getProximity(const pmPosition<float, DIM>& pos) {
...
void DiscreteWorld<DIM>::calculateWallProximity(int res) {
	...
	// Allocate new matrix.
	_wall_proximity = new pmMatrix<pmAcceleration<float, DIM>*>(w, h);
	// Calculate proximities.
	for(int y = 0; y < h; ++y) {
		for(int x = 0; x < w; ++x) {
			_wall_proximity->set(new pmAcceleration<float, DIM>, x, y);
...
void DiscreteWorld<DIM>::processDistanceToWall(int x, int y, int wx, int wy, int res, float& dist) {
	...
	pmAcceleration<float, DIM>* acc = _wall_proximity->get(x, y);
	pmPosition<float, DIM> *pb, *pw;
	// Decide which section we're in and determine distance.
	if(py < wy) {							// UPPER SECTION.
		if(px < wx) {						// Upper left.
			pb = new pmPosition<float, DIM>(px, py), pw = new pmPosition<float, DIM>(fwx, fwy);
		} else if(px > wx + 1) {			// Upper right.
			pb = new pmPosition<float, DIM>(px, py), pw = new pmPosition<float, DIM>(fwx + 1, fwy);
		} else {							// Upper center.
			pb = new pmPosition<float, DIM>(0, py), pw = new pmPosition<float, DIM>(0, fwy);
		}
	} else if(py >= wy && py <= wy + 1) {	// MIDDLE SECTION.
		if(px < wx) {						// Middle left.
			pb = new pmPosition<float, DIM>(px, 0), pw = new pmPosition<float, DIM>(fwx, 0);
		} else if(px > wx + 1) {			// Middle right.
			pb = new pmPosition<float, DIM>(px, 0), pw = new pmPosition<float, DIM>(fwx + 1, 0);
		} else {							// Middle center.
			//pb = new pmPosition<float, DIM>(px, py), pw = new pmPosition<float, DIM>(fwx + 0.5f, fwy + 0.5f);
			pb = new pmPosition<float, DIM>, pw = new pmPosition<float, DIM>;
		}
	} else {								// LOWER SECTION.
		if(px < wx) {						// Lower left.
			pb = new pmPosition<float, DIM>(px, py), pw = new pmPosition<float, DIM>(fwx, fwy + 1);
		} else if(px > wx + 1) {			// Lower right.
			pb = new pmPosition<float, DIM>(px, py), pw = new pmPosition<float, DIM>(fwx + 1, fwy + 1);
		} else {							// Lower center.
			pb = new pmPosition<float, DIM>(0, py), pw = new pmPosition<float, DIM>(0, fwy + 1);
		}
	}
	// Handle changes.
	d = pb->distance(*pw);
